...
# utilized libraries implemented
import model_util as mu
import predictor
import data

# set neural network model directory position and its name
overall_title = 'reModelSeq2Seq_tune'
title = 'boringssl_'+overall_title+'_tryOVF'

# get neural network model according set directory and name
model = mu.getModel(overall_title, title)


# initiate device [set for gpu utilization]
device = None
...
def getJson(fn):
    comDir = '/home/yangheechan/code_gen_web/flask/c2tok'
    flaskDir = '/home/yangheechan/code_gen_web/flask' 

    os.chdir(comDir)

    cmd = './query_gen.py ' + fn

	# execute query_gen.py program to extract token information from c source code
...
def getidentifier2Str(patch_list):
    genDir = '/home/yangheechan/code_gen_web/flask/identifier_gen'
    flaskDir = '/home/yangheechan/code_gen_web/flask' 
    ...
    for patch_dict in patch_list:
		...
		# execute python3 identifier_gen.py to generate string for identifiers
        stdout = subprocess.check_output(cmd, stderr=subprocess.STDOUT, shell=True, encoding='utf-8')
...
def generate():
	...
	# receive source code input from web browser [front-end]
    webData = request.get_json()
    source_code = webData['text']

	# make file on server directory ./c2tok...
	# to compile c file and extract lexical token
    fn = makeFile(source_code)
        
	# extract token information as json format
	# from token generated by ./c2tok/query_gen.py
    json_data = getJson(fn)


	# predict sequence of tokens
	# produce list of sequence of tokens by myBeamStart()
	...
	# cleans token sequence mixed around zero
    pred_results = data.strip(pred_results)

	# adds 0 to match length 10 tokens
    final_json = data.returnFinalJson(json_data, pred_results)

	# bring string for tokens predicted as identifier
	# utilizes pointer network model
    total_colored = getidentifier2Str(final_json)

	# colors tokens with fixed string according to dictionary
	# colors identifiers according to pointer network results (likelihood)
